var playExtreme_state = {
    ...
    create: function() {        
      ...
      dframe=0;
        ...
        duckStatus=true;
        bgm = this.game.add.audio('gamebgmEX',1,true);
        bgm.play('',0,1,true);
        bgm.volume=volumebgm;
        this.skys = game.add.group();

           this.skys.createMultiple(20,'skyNight');
    ...
    update: function() {
      ...
      else if (game.input.keyboard.isDown(Phaser.Keyboard.DOWN)) 
      {
        ...
        ducks.forEach(function(item) {
          item.body.y=230+Math.sin(frame/40)*80*level;
        });

        ducks3.forEach(function(item) {
          item.body.y=400+Math.cos(frame/40)*160;
        });

        ducks2.forEach(function(item) {
          item.body.y=230+Math.tan(frame/(100/level))*50*level;
        });
        ...
        var counter=0;
          if(Math.random()*1000<10&&duckStatus&&alive){
           ...
           for(var indent=0;counter<level*2;indent+=Math.floor(Math.random()*4+2)*100)
           {
             if(Math.random()<0.55)
             {
              var duck = ducks.create(1280+indent,500,'duckNight');
            ...
            {
             var duck = ducks3.create(1280+indent,30+Math.random()*170,'duckNight');
             duck.animations.add('fly',[0,1,2,3],7,true);
             duck.animations.play('fly');
             duck.scale.setTo(0.6,0.6);
           ...
           if(Math.random()<0.1)
           {
            if(Math.random<0.5)
            {
             var duck = ducks.create(1280+indent,30+Math.random()*170,'duckNight');
             duck.animations.add('fly',[0,1,2,3],7,true);
             duck.animations.play('fly');
             duck.scale.setTo(0.6,0.6);
           ...
           {
            var duck = ducks3.create(1280+indent,500,'duckNight');
            duck.animations.add('fly',[0,1,2,3],7,true);
            duck.animations.play('fly');
            duck.scale.setTo(0.6,0.6);
            duck.body.velocity.x=-400*level;
    ...
    if (Math.random()*(2000/level)<10&&alive) {
     var duck = ducks2.create(1280,30+Math.random()*170,'duckNight');
     duck.animations.add('fly',[0,1,2,3],7,true);
     duck.animations.play('fly');
     duck.scale.setTo(0.6,0.6);
     duck.body.velocity.x=-(Math.random()*200+350);
        ...
        game.physics.overlap(this.juxing, ducks3,this.hitDucks, null ,this);
      ...
      if ((frame-dframe)%(72+level*48)==0) {
        ...
        if (alive) {
          ...
          if(frame%1200==1199){
           level++;
           levelaudio.play();
           this.levelUp=this.game.add.text(580, 300, 'Nightmare: ' + level, { fontSize: '32px', fill: '#FFFFFF' }); 
