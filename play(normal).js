    ...
    update: function() {
        ...
        var counter=0;
          if(Math.random()*1000<10&&duckStatus&&alive){
           ...
           for(var indent=0;counter<level;indent+=Math.floor(Math.random()*4+2)*100)
           {
            ...
            {
             var duck = ducks.create(1280+indent,30+Math.random()*200,'duck');
             duck.animations.add('fly',[0,1,2,3],7,true);
             duck.animations.play('fly');
             duck.scale.setTo(0.6,0.6);
      ...
      if ((frame-dframe)%(72+level*48)==0) {
        ...
        if (alive) {
          ...
          if(frame%1200==1199){
           level++;
           this.levelUp=this.game.add.text(620, 300, 'Level: ' + level, { fontSize: '32px', fill: '#666666' }); 
         }
         if(frame>1200&&frame%1200==240){
